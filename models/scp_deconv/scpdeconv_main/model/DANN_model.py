...
class DANN(object):
    ...
    def prepare_dataloader(self, source_data, target_data, batch_size):
        ...
        self.target_index = list(target_data.obs_names)
        if self.target_type in ("simulated", "external_simulated"):
    ...
    def prediction(self):
        ...
        target_preds = pd.DataFrame(preds, index=self.target_index, columns=self.labels)
        ground_truth = pd.DataFrame(gt, index=self.target_index, columns=self.labels)
