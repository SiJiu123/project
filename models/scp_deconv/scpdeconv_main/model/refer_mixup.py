...
class ReferMixup(object):
    ...
    def mixup(self):
        ...
        elif self.target_type == "external_simulated":
            target_data = self.load_external_simulated_data(
                self.target_dataset_name,
                self.target_metadata_name,
            )
    ...
    def load_external_simulated_data(self, dataset, metadata):
        target_data = self.load_real_data(dataset)
        if not metadata:
            print(f"Metadata file is not provided for external simulated target [cyan]{dataset}.")
            sys.exit(1)

        try:
            target_y = pd.read_csv(metadata, header=0, index_col=0)
        except FileNotFoundError as e:
            print(f"No such target metadata csv file found for [cyan]{metadata}.")
            sys.exit(e)

        missing_celltypes = [ct for ct in self.type_list if ct not in target_y.columns]
        if missing_celltypes:
            print(f"Target metadata is missing cell type columns: {missing_celltypes}")
            sys.exit(1)
